document.addEventListener('DOMContentLoaded', function () {
    ...
    // Activer/Désactiver le champ "Autre (précisez)" si la case "Autre" est cochée
    ...
    dispoAutreCheckbox.addEventListener('change', function () {
        autrePrecisezInput.disabled = !this.checked; // Active ou désactive le champ texte "Autre (précisez)" en fonction de la case à cocher
    });

    // Activer/Désactiver le champ "Autre (précisez)" en fonction de la case à cocher avec l'ID 'dispo_autre'
    const dispoAutreCheckboxInput = document.getElementById('dispo_autre'); // Cette ligne est spécifique à votre demande
    dispoAutreCheckboxInput.addEventListener('change', function () {
        if (this.checked) {
            dispoAutreCheckboxInput.disabled = false; // Active l'input quand 'dispo_autre' est coché
        } else {
            dispoAutreCheckboxInput.disabled = true;  // Désactive l'input si décoché
        }
    });
