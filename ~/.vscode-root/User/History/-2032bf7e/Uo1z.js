document.addEventListener('DOMContentLoaded', function () {
    ...
    rappelCheckbox.addEventListener('change', function () {
        disponibiliteCheckboxes.forEach(cb => {
            cb.disabled = this.checked; // Désactive les cases à cocher si "Être rappelé(e)" est coché
        });
    });
    ...
    autreCheckbox.addEventListener('change', function () {
        ...
        if (this.checked) {
            autreInput.disabled = false; // Active le champ si "Autre" est coché
        } else {
            autreInput.disabled = true; // Désactive le champ si "Autre" n'est pas coché
    ...
    document.getElementById('contactForm').addEventListener('submit', function (event) {
        ...
        .then(data => {
            ...
            if (data.includes('Votre message a été envoyé avec succès.')) {
                ...
                autreInput.disabled = true; // Désactive le champ "Autre" après réinitialisation
            }
        })
        .catch(error => {
